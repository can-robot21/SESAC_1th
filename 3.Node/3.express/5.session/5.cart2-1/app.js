...
app.get('/increateProduct', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'cartBox.html'));
})

app.get('/discreatsProduct', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'cartBox.html'));
})
...
//     const productId = parseInt(req.params.poructId);
//     const change = parseInt(req.params.change);

//     if (isNaN(productId)) || (isNaN(change)) {
//         return res.status(400).json(massege: '잘못된 요청입니다.');
//     }
